...
export function parseJSX(jsxCode: string): ParseResult {
  console.log('RAN');
  const functionPattern =
    /(?:(?<=\(\)\s*=>|=>\s*)\s*(\w+)\s*\(|(\w+)\s*=\s*\([^)]*\)\s*=>|on\w+={(\w+)})/g;
  ...
  const functions: FunctionInfo[] = Array.from(functionsSet).map((name) => {
    const argCount = (
      jsxCode.match(new RegExp(`${name}\\s*\\(([^)]*)\\)`, 'g')) || []
    )
      .map(
        (call) =>
          call
  ...
  const tests = [
    {
      ...
      jsx: `
        ...
        `,
      expected: {
        functions: [{ name: 'updateCount', argCount: 1, argTypes: ['any'] }],
        variables: ['count'],
      },
    },
    {
    ...
    },
    {
      ...
      jsx: `
        ...
        `,
      expected: {
        functions: [{ name: 'updateColor', argCount: 1, argTypes: ['any'] }],
        variables: ['color'],
      },
    },
    {
    ...
    },
    {
      ...
      jsx: `
        ...
        `,
      expected: {
        functions: [{ name: 'updateIndex', argCount: 1, argTypes: ['any'] }],
